...
export const banUnderscorePrefixOnPublicFieldRule: TSESLint.RuleModule<
...
> = {
  meta: {
    docs: {
      description:
        "ban underscore(_) prefix on public field for more friendly mangling-properties by terser",
      recommended: "warn",
    ...
    messages: {
      banUnderscorePrefixOnPublicField:
        "Public member '{{fieldName}}' is should not be prefixed with _ for more friendly mangling-properties by terser",
    },
    schema: [
      {
    ...
    return {
      PropertyDefinition(node): void {
        ...
        if (name.toString().startsWith("_")) {
          report({
            node,
            messageId: "banUnderscorePrefixOnPublicField",
            data: {
              fieldName: name,
            },
          });
